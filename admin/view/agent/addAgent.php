...
if (isset($_POST['submit'])) {
    ...
    else 
    {
        ...
        $addStatus = $agent_controller->addAgent($data);

        if ($addStatus) 
        {
            echo '<script>location.href="agent.php?addStatus='.$addStatus.'"</script>';
        }
            ...
            <div class='col-md-12'>
                <form action='' method='POST'>
                    ...
                    <div class="mb-3">
                        ...
                        <input type='text' name='phone' value="<?php if (isset($_POST['phone']))  echo $_POST['phone']; ?>" class='form-control'>
                        <?php if (isset($phoneError)) echo '<span class="text-danger">' . $phoneError . '</span>' ?>
                    </div>

                    <div class="mb-3">
                        <label class='form-label'>Facebook link</label>
                        <input type='text' name='facebook' value="<?php if (isset($_POST['facebook']))  echo $_POST['facebook']; ?>" class='form-control'>
                        <?php if (isset($facebookError)) echo '<span class="text-danger">' . $facebookError . '</span>' ?>
                    </div>

                    <div class="mb-3">
                        <label class='form-label'>Viber</label>
                        <input type='text' name='viber' value="<?php if (isset($_POST['viber']))  echo $_POST['viber']; ?>" class='form-control'>
                        <?php if (isset($viberError)) echo '<span class="text-danger">' . $viberError . '</span>' ?>
                    </div>

                    <div class="mb-3">
                        <label class='form-label'>Telegram User Name</label>
                        <input type='text' name='telegram_user_name' value="<?php if (isset($_POST['telegram_user_name']))  echo $_POST['telegram_user_name']; ?>" class='form-control'>
